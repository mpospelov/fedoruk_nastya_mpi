...
int main(int argc, char **argv) {
  ...
  while(currentTime < modelingTime) {
    ...
    for(i = 0; i<sub_nodes; i++) {
      ...
      if(i == 0) {
        T_n1[i] = a_t*(left_b - 2*T_n1[i] + T_n1[i + 1])*deltaT/(deltaX*deltaX) + T_n1[i];
        continue;
      }

      if(i == sub_nodes - 1) {
        T_n1[i] = a_t*(T_n1[i - 1] - 2*T_n1[i] + right_b)*deltaT/(deltaX*deltaX) + T_n1[i];
        continue;
      }

      if((i > 0) && (i < sub_nodes-1)) {
        T_n1[i] = a_t*(T_n1[i - 1] - 2*T_n1[i] + T_n1[i + 1])*deltaT/(deltaX*deltaX) + T_n1[i];
        //printf("RANK %d is working %d from %d is %f\n", myrank, i, sub_nodes, T_n1[i]);
      }

      #ifdef PRINT_TO_FILE
        if(i % 10 == 0) {
          MPI_Gather((void *)T_n1, sub_nodes, MPI_DOUBLE, (void *)t_n1, sub_nodes, MPI_DOUBLE, 0, MPI_COMM_WORLD);
